$("#fileUpload").on('change', function () {
 ...
 $('table').on('click','.edit',function(){
     $('#add_input').empty();
      ...
      var aValue= oTr.children('.b_id').text();
      $('#add_input').prepend('<input type="hidden" name="blockid" id="blockid" value="'+aValue
        +'"/>'
 ...
 $('table').on('click','.fa-trash-o',function(){
     var oDelete = $(this);
    var bId=$(this).parents('tr.table_con').children('.b_id').text();
    data = {
        'method': 'delete',
    ...
    $.ajax({
        type: 'post',
        url: '/sitecontent/blockcontents/?new',
        data:data,
